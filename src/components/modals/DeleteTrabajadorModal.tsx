 ...
 */
export const DeleteTrabajadorModal = ({
    isOpen,
    onClose,
    onConfirm,
    TrabajadorData
}: DeleteTrabajadorModalProps) => {
    // Estado para manejar el ID ingresado por el usuario
    const [nodoId, setNodoId] = useState(TrabajadorData?.ID_Trabajador?.toString() || '');
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            if (result.ok) {
                ...
                setSuccess(result.message || `Trabajador eliminado exitosamente`);
                setTimeout(() => {
                    onClose();
                }, 1000);
            } else {
        ...
        <Modal isOpen={isOpen} onClose={onClose} className="max-w-[500px] m-4">
            <div className="relative w-full p-4 bg-white rounded-xl dark:bg-gray-900">
                ...
                <div className="mb-6">
                    <div className="mb-4">
                        <Label>
                            ID del Trabajador <span className="text-red-500">*</span>
                        </Label>
                        <TextArea
                            value={nodoId}
                            onChange={(value) => {
