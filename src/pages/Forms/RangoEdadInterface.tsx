...
import { CreateRangoEdadModal } from "../../components/modals/CreateRangoEdadModal";
import { UpdateRangoEdadModal } from "../../components/modals/UpdateRangoEdadModal";
import { DeleteRangoEdadModal } from "../../components/modals/DeleteRangoEdadModal";

/**
 * Interfaz principal para la gestión de Nodos IoT.
 * Permite crear, actualizar y eliminar nodos IoT en la base de datos.
 */
export default function RangoEdadInterface() {
  ...
  const handleCreate = async (formData: any) => {
    try {
      const result = await registrar_rangoedad(formData);
      handleRefresh(); // Actualizar la tabla después del éxito
      return {
        ok: true,
        message: "rango edad registrado exitosamente",
        data: result
      };
    } catch (error: any) {
      return {
  ...
  const handleUpdate = async (formData: any) => {
    try {
      const result = await actualizar_rangoedad(formData);
      handleRefresh(); // Actualizar la tabla después del éxito
      return {
        ok: true,
        message: "rango edad actualizado exitosamente",
        data: result
      };
    } catch (error: any) {
      return {
        ok: false,
        message: error.message || "Error al actualizar el rango edad",
        data: null
      };
    }
  };
  ...
  const handleDelete = async (formData: any) => {
    try {
      await eliminar_rangoedad(formData);
      handleRefresh();
      return {
        ok: true,
        message: "rango edad eliminado exitosamente"
      };
    } catch (error: any) {
      return {
        ok: false,
        message: error.message || "Error al eliminar el rango edad"
      };
    }
  };
  ...
  return (
    <div>
      ...
      <div className="grid">
        ...
        <UpdateRangoEdadModal
          isOpen={isUpdateOpen}
          onClose={closeUpdateModal}
          onSave={handleUpdate}
          initialData={selectedRangoEdad}
        />

        {/* Modal para eliminar */}
        <DeleteRangoEdadModal
          isOpen={isDeleteOpen}
